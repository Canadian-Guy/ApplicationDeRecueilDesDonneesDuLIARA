...
Users.countDocuments(function(error, count){
    ...
    if(!error && count === 0){
        ...
        admin.save(function(error, dummy){
            if(error) return console.error(error);
...
app.post('/users', (req, res) =>{
    ...
    res.status(200).json({UserName: userName, Password: password, message: "received user with username " + userName + " and password " + password});
});
